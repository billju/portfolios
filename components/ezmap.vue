...
function Bounds(minPoint, maxPoint){
...
Bounds.prototype = {
    ...
    getSize(){return new Point(this.max.x-this.min.x, this.max.y-this.min.y)},
}
...
function Map(element,options){
    ...
    this.svg.padding = window.navigator.userAgent.match(/MSIE|Edge|Firefox/)?0:0.1,
    this.resetSvgSize()
...
Map.prototype = {
    event: null, gesture: null, moved: true,
    handleResize(){
        if(this.smoothFinished){
    ...
    zoom: 2,
    limitZoom(zoom){return Math.max(this.options.minZoom,Math.min(this.options.maxZoom,zoom))},
    scale: new Point(0,0),
    Point: new Point(0,0),
    Bounds: new Bounds({x:0,y:0},{x:0,y:0}),
    LatLng: new LatLng(0,0),
    LatLngBounds: new LatLngBounds({lat:0,lng:0},{lat:0,lng:0}),
    layer: {},
